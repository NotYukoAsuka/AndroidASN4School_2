...
public class MineTests {
    ...
    public void test_playing() throws Exception {
        ...
        int message_found = testing_game.msg_found_mine();
        int message_scanned = testing_game.msg_already_scanned();
        int number = testing_game.click_mines(0);
        if (number == message_found) {
            Assert.assertEquals(testing_game.getScanCount(), 0);
        ...
        boolean scanned = testing_game.checkIfScanned(0);
        ...
        if (!scanned) {
            ...
            Assert.assertEquals(new_number, message_scanned);
            Assert.assertEquals(testing_game.getScanCount(), 1);
        }
        Assert.assertEquals(testing_game.getIndexStatus(0), false);
        int scanned_mine_total = 0;
        ...
        Assert.assertEquals(result, message_found);
        if (number == message_found) {
    ...
    @Test
    public void scan_status_check() throws Exception {
        Mines testing_game = Mines.getInstance(10, 10, 20);
        Assert.assertEquals(testing_game.checkIfScanned(0), false);
        testing_game.removeMine(0);
        int result = testing_game.click_mines(0);
        Assert.assertEquals(result, testing_game.getScannedMineCRCount(0));
        Assert.assertEquals(testing_game.checkIfScanned(0), true);
        Assert.assertEquals(testing_game.getScanCount(), 1);
        result = testing_game.click_mines(0);
        Assert.assertEquals(result, testing_game.msg_already_scanned());
        Assert.assertEquals(testing_game.getScanCount(), 1);
    }
    ...
    @Test(expected = IllegalArgumentException.class)
    public void testing_bad_cases_10() throws Exception {
        Mines testing_game_again = Mines.getInstance(2, 2, 1);
        testing_game_again.checkIfScanned(-1);
    }
}